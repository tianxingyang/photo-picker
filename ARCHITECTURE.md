       ...
       └─> invoke('export_keep', { destDir })
            └─> Rust export_keep:
                 ...
                 ③ spawn_blocking #2（不持 DB 锁）：逐张 resolve_target 探空位 → std::fs::copy
                      · 源已在目标目录内（源文件父目录==目标）→skipped++（不复制，避免自我克隆）
                        判定两级：canonicalize 规范化比较；canonicalize 不可用的文件系统
                        （WebDAV/部分 NAS）回退到词法归一比较（要求源文件存在，已删除的
                        keep 仍走 failed，不伪装成 skip）
                      · 目标重名→ name (n).ext（renamed++）
                      · 成功→exported++；单项失败/源无文件名/源已删除/名额耗尽→failed.push（不中断）；
                        copy 失败时清理可能写了一半的目标文件，重试不会被残留截断文件挤到 (n) 后缀
...
源原片全程只读：`std::fs::copy` 只读源、写新目标，绝不 rename/remove/写回源路径（失败清理只删除
本次 copy 自己刚创建的目标文件，源不受影响）；`resolve_target` 只在探到「目标不存在」时才写，
故不覆盖目标已有同名文件。
