...
- [x] 7 个子任务全部 `completed` 并归档。
- [x] 在真实连拍文件夹（含 JPG/PNG/HEIC、含明显虚焦/废曝光、含多组近重复）跑通完整链路。（2026-06-13 用户 GUI 实测通过）
- [x] 导入后 `photos` 表含全部文件行，HEIC 也被识别。
- [x] 分析完成后每张有 `shot_at`（EXIF 存在时）/ `blur` / `exposure` 结果；明显废片被标出。（DB 实测：126/126 分析列齐全，52 张标模糊、1 张欠曝）
- [x] 近重复连拍聚到同一组，组数与肉眼判断大致吻合。（DB 实测：20 个 `phash_burst` 组，组大小 2–6）
- [x] 组内可浏览、可 A/B 对比、键盘 1/2 选保留。
- [x] 保留/淘汰/待定状态落库，重启应用后保持。
- [x] 导出后目标目录出现且仅出现 `status=keep` 的原片副本，文件名保留，源文件未被改动。
- [x] `ROADMAP.md` Milestone 1 的 10 项功能全部可用。

> 集成验收于 2026-06-13 完成：自动化部分（Rust 61 tests / Python 25 tests / tsc+vite build / clippy `-D warnings` / ruff 全过，DB 只读核查迁移 v3 + 分析/分组/状态落库），GUI 端到端链路由用户实测通过。
