...
- [x] 缩略图后台批量预生成 + WebP 缓存（扫描后自动链式，单档 512px WebP，mtime 自愈，网格/对比 filmstrip 改用缩略图、HEIC 首次可显示真实预览）— 交付 2026-06-15
- 时间线视图（按拍摄时间轴聚合）
- 快捷键全覆盖（导航、状态切换、对比）
- 撤销 / 重做（保留-淘汰的操作历史）
...
- [x] 缩略图分辨率档位（一档 256? 双档 256/512?）— **单档 512px WebP@q80**（M2，2026-06-15）：实测网格瓦片 180–270px 方形、retina 最坏 540 物理像素，512px 恰好压住且对 64px filmstrip 绰绰有余；前端零 srcset/DPR，双档不抵复杂度。大库 profiling 证明过重再起任务。
