...
- [ ] 数据模型：宽表 vs 高表（影响后续所有 SQL）
- [ ] 状态字段：TEXT 枚举 vs INT 编码
- [ ] 模糊阈值：硬编码 vs 用户可调 vs 组内归一
- [ ] 相似分组的时间窗口宽度
- [ ] 组内 keep 时是否自动 reject 其它
- [ ] 缩略图分辨率档位（一档 256? 双档 256/512?）
- [ ] HEIC 解码是否走 sidecar 还是 Rust 端用 libheif crate
